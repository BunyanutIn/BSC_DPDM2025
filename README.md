...
Decision Tree Regressor คืออัลกอริทึมในกลุ่ม Machine Learning ที่ใช้สำหรับ ทำนายค่าที่เป็นตัวเลขต่อเนื่อง (Regression) เช่น การพาดพิงราคาบ้าน, ปริมาณน้ำฝน หรือยอดขายในอนาคต โดยเลียนแบบกระบวนการตัดสินใจของมนุษย์ในลักษณะของ "โครงสร้างต้นไม้" ถ้าจะให้เห็นภาพง่ายที่สุด ลองนึกว่าคุณกำลังเล่นเกมทายตัวเลข แล้วคุณถามคำถามแบบ "ใช่/ไม่ใช่" ไปเรื่อยๆ จนกว่าจะได้คำตอบที่ใกล้เคียงที่สุดนั่นเอง
  ...
  1. โครงสร้างและการทำงานDecision Tree จะทำการแบ่งข้อมูลออกเป็นส่วนย่อยๆ (Subsets) ตามเงื่อนไขของฟีเจอร์ที่เรามี จนกระทั่งได้กลุ่มข้อมูลที่เล็กลงเรื่อยๆRoot Node (โหนดราก): จุดเริ่มต้นที่เป็นเงื่อนไขแรกสุดDecision Node (โหนดตัดสินใจ): จุดที่มีการแยกสาขาตามเงื่อนไข (เช่น ถ้าค่า $X > 10$ ให้ไปทางซ้าย)Leaf Node (โหนดใบ): จุดสิ้นสุดของการทำนาย ซึ่งในกรณีของ Regressor ค่าที่ได้จะเป็นค่าเฉลี่ย (Mean) ของข้อมูลทั้งหมดที่ตกอยู่ในโหนดนั้น

  2. เขาตัดสินใจ "ตัด" ข้อมูลตรงไหน?ในขณะที่ Decision Tree สำหรับจำแนกประเภท (Classifier) จะใช้ค่า Gini หรือ Entropy แต่สำหรับ Regressor เขาจะมองหาจุดที่ทำให้ ความผิดพลาด (Error) น้อยที่สุดตัวชี้วัดที่นิยมที่สุดคือ Mean Squared Error (MSE):$$MSE = \frac{1}{n} \sum_{i=1}^{n} (y_i - \bar{y})^2$$$y_i$: ค่าจริงของข้อมูล$\bar{y}$: ค่าเฉลี่ยของข้อมูลในโหนดนั้น (ซึ่งคือค่าทำนาย)อัลกอริทึมจะพยายามหาทางตัด (Split) ข้อมูลที่ทำให้ผลรวมของ MSE ในโหนดลูก (Child Nodes) ต่ำที่สุดเท่าที่จะเป็นไปได้ครับ
     
  4.  ข้อดีและข้อเสียที่ควรรู้ข้อดีข้อเสียเข้าใจง่าย (Interpretability): สามารถวาดออกมาเป็นภาพและอธิบายให้คนทั่วไปเข้าใจได้ง่ายOverfitting ง่ายมาก: ถ้าปล่อยให้ต้นไม้โตเกินไป มันจะจำข้อมูลเก่าแม่นมาก แต่ทำนายข้อมูลใหม่พลาดไม่ต้องเตรียมข้อมูลเยอะ: ไม่ต้องทำ Scaling ข้อมูล (เช่น Normalization) ก็ทำงานได้ดีความไม่เสถียร (Instability): เปลี่ยนข้อมูลแค่เล็กน้อย รูปทรงต้นไม้อาจเปลี่ยนไปคนละเรื่องเลยรับมือกับ Non-linear ได้ดี: จัดการกับความสัมพันธ์ที่ซับซ้อนของข้อมูลได้เก่งไม่เก่งเรื่อง Extrapolation: ไม่สามารถทำนายค่านอกเหนือจากช่วงของข้อมูลที่เคยเห็นตอน Train ได้
